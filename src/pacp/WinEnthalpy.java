...
public class WinEnthalpy {
	...
	/* ----------------------------------------
	 *  	STATIC GLOBAL VAR PRIVAT
	 * ---------------------------------------- */
	// JPopupMenu's Position
	private static Point pointJPopupMenu;
	...
	public int getIdNearest(List<ElDraw> eDrawL, double pH, double pP, double zoom) {
		...
		for(int i=0;i<eDrawL.size();i++) {
			...
			if ( eDrawL.get(i).getType() == ElDraw._PointYLog) {
				...
				//System.out.println("point H = " + pH);
				//System.out.println("H+zoneH = " + ((H+zoneH)/zoom));
				//System.out.println("H-zoneH = " + ((H-zoneH)/zoom));
				//System.out.println("Point P = " + pP);
				//System.out.println("P+zoneP = " + ((P+zoneP)/zoom));
				//System.out.println("P-zoneP = " + ((P-zoneP)/zoom));
	...
	/**
	 * Generated by Win builder to create the Popup menu in the panel (Delete Point/Move Point)
	 */
	...
	private void initialize() {
		...
		mntmDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				//System.out.println("Delete position: H=" +  panelEnthalpyDrawArea.getHoXm((int)pointJPopupMenu.getX()) + "   P=" + panelEnthalpyDrawArea.getPoYm((int)pointJPopupMenu.getY()));
				//int id = getIdNearest(eDrawL, panelEnthalpyDrawArea.getHoXm((int)pointJPopupMenu.getX()), panelEnthalpyDrawArea.getPoYm((int)pointJPopupMenu.getY()), panelEnthalpyDrawArea.getZoom());
				int id = getIdNearest(eDrawL, panelEnthalpyDrawArea.getHoXm((int)pointJPopupMenu.getX()), panelEnthalpyDrawArea.getPoYm((int)pointJPopupMenu.getY()), 1);
				if (id >= 0) {
					eDrawL.remove(id);
					panelEnthalpyDrawArea.repaint();					
				}
